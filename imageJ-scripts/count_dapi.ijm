close("*");
setBatchMode("hide");
input = "/Volumes/4TB/96WP/96WP_Data/C1";
output = "/Users/samuelorion/Documents/GitHub/2020_Manuscript/R/DATA-OUT/96WP/DAPI/";
File.makeDirectory(output);
images_to_open = getFileList(input); 

for (i = 165; i < images_to_open.length; i++){
	file = input + "/" + images_to_open[i];
	t = getTime();
	image_name = replace(images_to_open[i],".tif","");
	print(image_name);  
	//run("Bio-Formats", "open=file autoscale color_mode=Default crop rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT x_coordinate_1=1000 y_coordinate_1=1000 width_1=3000 height_1=3000");
	open(file);
	//run("mpl-viridis");
