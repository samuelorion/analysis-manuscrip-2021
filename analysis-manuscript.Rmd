...
## PD-vulnerable neurons are more vulnerable to hydrogen peroxide than pd-resilient neurons
...
```{r echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, comment = NA, out.width="50%"}
stat.test1 <- aov(neuron_normalized ~ type + hydrogen_peroxide, data = data)
stat.test2 <- 
  data %>% 
  select(hydrogen_peroxide, type, neuron_normalized) %>%
  group_by(hydrogen_peroxide) %>%
  pairwise_t_test(
    neuron_normalized ~ type, paired = FALSE, 
    p.adjust.method = "bonferroni"
    ) 
...
Kruskal-Wallis and Dunn test for each
...
## No overt difference in mitochondrial ROS production is observed between neurons
...
## Distance between mitochondria from roGFP experiments
...
Grouped by vulnerable and non-vulnerable
...
plot(two.group.unpaired.meandiff, color.column = type)
ggsave(here("plots", "apercentage_syt1_positive_PD_nonPD-Gardner-Altman plot.pdf"))
```
...
PD-vulnerable vs Non-vulnerable
